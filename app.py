...
app = Flask(__name__)
CORS(app)  
api = Api(app, version='1.0', title='OPEN AI',
          description='Genrating the Endpoints from openai')

# Swagger namespace
ns = api.namespace('Openai', description='Genrating the Endpoints from openai')
...
parser.add_argument('user_input', type=str, help='User input for chat', location='args')  # Added user_input
...
class ChatResource(Resource):
    ...
    def post(self):
        ...
        # Use user_input from the parser
        ...
        if confirmation == 'yes':
            ...
            # Insert into MongoDB
            if extracted_data:
                inserted_ids = [str(item) for item in collection.insert_many(extracted_data).inserted_ids]
                print("Data inserted into MongoDB. Inserted IDs:", inserted_ids)

            return jsonify({"message": "Success", "data": inserted_ids})
        else:
